...
class Auth extends React.Component {
  ...
  login = (e) => {
    ...
    if (email.length < 1 || password.length < 1) {
      Swal.fire("Register Failed", "All field must be filled", "error");
    }
    ...
    login
      .then((res) => {
        ...
        Swal.fire("Login Success", "Redirect to catalog", "success").then(
          () => {
            this.setState({
              isLoading: true,
            });
      ...
      .catch((rej) => {
        Swal.fire("Login Failed", rej.response.data.msg, "error");
      })
      .finally(() => {
        this.setState({
          isLoading: false,
  ...
  register = (e) => {
    ...
    regis
      .then((res) => {
        Swal.fire("Register Success", "Redirect to login", "success").then(
          () => {
            this.setState({
              isLoading: true,
            });
    ...
    return (
      <>
        <div
          ...
          className="position-fixed rounded-circle bg-dark text-white m-3 d-flex align-items-center justify-content-center"
          style={{
            width: "35px",
            height: "35px",
            cursor: "pointer",
        ...
        <Card
          color="white"
          className="auth-box mx-auto shadow-sm p-3 border-0 rounded-0"
        >
                ...
                <Route exact path="/auth/login">
                  ...
                  <form onSubmit={this.login}>
                    ...
                    <div className="d-flex flex-column align-items-center mt-4">
                      <Button
                        type="submit"
                        className="px-4 rounded-pill cta border-0 align-self-stretch"
                      >
                        Login
                      </Button>
          ...
          {this.state.isLoading && (
            <div
              className="spinner-border text-main my-5 mx-auto"
              role="status"
            >
