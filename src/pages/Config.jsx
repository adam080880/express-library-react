...
class Config extends React.Component {
  ...
  searchAuthor = (e) => {
    e.preventDefault();
    this.setState({
      authors: this.state.realAuthors
        .map((val) => Object({ ...val, ...{ name: val.name.toLowerCase() } }))
        .filter((obj) => obj.name.includes(this.state.searchAuthor))
        .map((val) =>
          Object({
            ...val,
            ...{
  ...
  searchGenre = (e) => {
    e.preventDefault();
    this.setState({
      genres: this.state.realGenres
        .map((val) => Object({ ...val, ...{ name: val.name.toLowerCase() } }))
        .filter((obj) => obj.name.includes(this.state.searchGenre))
        .map((val) =>
          Object({
            ...val,
            ...{
  ...
  addAuthor = (e) => {
    ...
    AuthorModel.post({ name, description })
      .then((res) => {
  ...
  editAuthor = (e) => {
    ...
    AuthorModel.patch({ name, description, id })
      .then((res) => {
    ...
    <tr key={index}>
      ...
      <td>
        ...
        <button
          className="btn btn-warning btn-sm ml-2"
          onClick={(e) =>
            this.trigger("editGenre", { id: val.id, name: val.name })
          }
        >
          <span className="fas fa-pen"></span>
        </button>
    ...
    return (
      <div className="container-fluid p-0 mb-5" style={{ overflowX: "hidden" }}>
        <Modal isOpen={this.state.isOpen} toggle={this.toggleModal}>
          {(this.state.form === "addAuthor" ||
            this.state.form === "editAuthor") && (
            <ModalBody>
              <form
                onSubmit={
                  this.state.form === "addAuthor"
          ...
          {(this.state.form === "addGenre" ||
            this.state.form === "editGenre") && (
            <ModalBody>
              <form
                onSubmit={
                  this.state.form === "addGenre"
              ...
              <div className="card-body">
                <div className="d-flex flex-row justify-content-between align-items-center">
                  ...
                  <div className="control d-flex align-items-center">
                    ...
                    <button
                      ...
                      onClick={(e) => this.trigger("addAuthor")}
                    >
                ...
                <div style={{ overflowX: "auto", overflowY: "auto" }}>
                  <table className="mt-2 w-100 table mt-4">
                    ...
                    <tbody>{this.state.authors.map(this.renderAuthor)}</tbody>
                  </table>
                </div>
              </div>
            </div>
              ...
              <div className="card-body">
                <div className="d-flex flex-row justify-content-between align-items-center">
                  ...
                  <div className="control d-flex align-items-center">
                    ...
                    <button
                      className="btn btn-primary btn-sm d-flex align-self-stretch align-items-center justify-content-center ml-2"
                      onClick={(e) => this.trigger("addGenre")}
                    >
                ...
                <div style={{ overflowX: "auto", overflowY: "auto" }}>
                  <table className="mt-2 w-100 table mt-4">
                    ...
                    <tbody>{this.state.genres.map(this.renderGenre)}</tbody>
                  </table>
                </div>
              </div>
            </div>
