...
class User extends React.Component {
  ...
  search = e => {
    ...
    const param = qs.stringify({ ...qs.parse(query), ...{search: this.state.user}  })
  ...
  toggleRole = (id) => {
    ...
    UserModel.toggleRole(id).then((res) => {
                  ...
                  <div className="d-flex flex-row align-items-center justify-content-between w-100 px-4">
                    ...
                    <div className="wrapper">
                      {[...Array(this.state.pageInfo.totalPage)].map((o, i) => (
                        <button onClick={()=>{this.fetchUser(qs.stringify({...params, page: params.page? i+1 : i+1})); this.props.history.push(`/dashboard/users?${qs.stringify({...params, ...{page: i+1}})}`)}} className='mx-1 btn btn-outline-secondary' key={i.toString()}>{i+1}</button>
                      ))}
                    </div>
                    <div className="btn-wrapper">
